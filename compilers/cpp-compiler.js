...
const CppCompiler = (() => {
  function toCanonical(src) {
    ...
    // cout << a << b << endl;   (cout concatenates operands with NO
    // separator, unlike print()'s space-joining — each operand is
    // wrapped in str() and joined with '+' so it collapses to one arg)
    body = body.replace(/cout\s*((?:<<\s*[^;]+?)+);/g, (m, chain) => {
      const parts = AdapterUtils.splitTopLevel(chain, '<<');
      let nl = false;
      const args = parts.filter(p => {
        if (p === 'endl' || p === 'std::endl') { nl = true; return false; }
        return true;
      });
      const joined = args.length ? args.map(a => `str(${a})`).join(' + ') : '""';
      return (nl ? 'print(' : 'printraw(') + joined + ');';
    });
    // cin >> a >> b;
