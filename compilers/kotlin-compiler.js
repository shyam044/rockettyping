...
const KotlinCompiler = (() => {
  ...
  // treatment (println/readLine, ranges, val/var, arrayOf, .size).
  function translateBody(body) {
    ...
    body = body.replace(/\b(?:arrayOf|listOf|mutableListOf)\s*\(([^()]*)\)/g, '[$1]');
  ...
  function toCanonical(src) {
    ...
    // Kotlin already requires parens around if/while conditions, so no
    // wrapBareConditions pass is needed here (unlike Go/Rust/Swift).
