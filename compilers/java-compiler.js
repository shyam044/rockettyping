...
const JavaCompiler = (() => {
  function toCanonical(src) {
    ...
    body = body.replace(/new\s+(?:int|long|short|byte|float|double|boolean|char|String)\s*\[\s*([^\]]+)\s*\]/g, 'array($1)');
    body = body.replace(/new\s+(?:int|long|short|byte|float|double|boolean|char|String)\s*\[\]\s*\{([^}]*)\}/g, '[$1]');
    // `int[] arr = {1,2,3};` — array-literal declaration without `new`
    body = body.replace(/\b(?:int|long|short|byte|float|double|boolean|char|String)\s*\[\s*\]\s*([A-Za-z_]\w*)\s*=\s*\{([^}]*)\}\s*;/g, '$1 = [$2];');
